...
const Remates = () => {
    React.useEffect(() => {
        document.title = "Puebla Motors | Remates"
    }, [])

    useRegistroVisitaPagina(4)
    const classes = useStyles();
    const vehiculos = useGetAPI(`categoria/3`)
    ...
    return (
        <div className={`${classes.root} animate__animated animate__fadeIn`}>
            <Typography className="to-top grid" variant="h4" gutterBottom>
                Veh&iacute;culos en remate {vehiculos.data.length > 0 && `(${vehiculos.data.length})`}
            </Typography>
            <Divider />
            {
